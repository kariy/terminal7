...
Discord users can link their Discord account to a system user via a web-based flow. This is used by the Discord bot to authenticate users before responding.

**Flow:**
1. Unlinked Discord user mentions the bot
2. Bot replies with a link URL containing a one-time code (expires in 10 minutes)
3. User opens the URL → if not logged in, redirected to log in first
4. User confirms the link on the confirmation page
5. Discord user ID is permanently mapped to the auth user
6. Bot now responds to that Discord user normally
...
// Response 200
{ "ok": true, "discord_username": "user#1234" }
```
...
| Column            | Type    | Description                               |
| ----------------- | ------- | ----------------------------------------- |
| `discord_user_id` | TEXT PK | Discord user ID                           |
| `auth_user_id`    | TEXT FK | References `auth_users.id` (CASCADE)      |
| `created_at`      | INTEGER | Creation timestamp (epoch ms)             |

Maps a Discord user to an auth user. Created via the web-based linking flow.

### `discord_link_codes`

| Column             | Type    | Description                              |
| ------------------ | ------- | ---------------------------------------- |
| `code`             | TEXT PK | One-time link code (UUID)                |
| `discord_user_id`  | TEXT    | Discord user ID requesting the link      |
| `discord_username` | TEXT    | Discord username (for display)           |
| `expires_at`       | INTEGER | Expiration timestamp (epoch ms)          |
| `created_at`       | INTEGER | Creation timestamp (epoch ms)            |

Temporary codes generated by the Discord bot. Expire after 10 minutes. Cleaned up periodically alongside expired auth sessions.
